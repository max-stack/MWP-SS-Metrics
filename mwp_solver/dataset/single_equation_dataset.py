...
from utils.enum_type import MaskSymbol, NumMask, SpecialTokens, FixType, Operators, EPT
from utils.enum_type import OPERATORS, SPECIAL_TOKENS
...
class SingleEquationDataset(AbstractDataset):
    ...
    def _preprocess(self):
        ...
        self.trainset, generate_list, train_copy_nums,unk_symbols = transfer(self.trainset, 'single_equation', self.mask_symbol, self.min_generate_keep,self.linear)
        self.validset, _g, valid_copy_nums,_ = transfer(self.validset, 'single_equation', self.mask_symbol, self.min_generate_keep,self.linear)
        self.testset, _g, test_copy_nums,_ = transfer(self.testset, 'single_equation', self.mask_symbol, self.min_generate_keep,self.linear)
