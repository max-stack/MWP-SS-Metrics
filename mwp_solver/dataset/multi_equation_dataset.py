...
from utils.preprocess_tool.equation_operator import from_infix_to_multi_way_tree, postfix_parser
from utils.preprocess_tool.equation_operator import from_infix_to_postfix, from_infix_to_prefix, from_postfix_to_infix, from_postfix_to_prefix, from_prefix_to_infix, from_prefix_to_postfix
from utils.preprocess_tool.sentence_operator import deprel_tree_to_file, get_group_nums_, span_level_deprel_tree_to_file, get_span_level_deprel_tree_, get_deprel_tree_
from utils.preprocess_tool.number_transfer import number_transfer
from utils.preprocess_tools import id_reedit,read_aux_jsonl_data,dataset_drop_duplication
from utils.preprocess_tools import preprocess_ept_dataset_
from utils.enum_type import MaskSymbol, Operators, SPECIAL_TOKENS, NumMask, SpecialTokens, FixType, EPT
...
class MultiEquationDataset(AbstractDataset):
    ...
    def _preprocess(self):
        ...
        self.trainset, generate_list, train_copy_nums,unk_symbol = transfer(self.trainset, 'multi_equation', self.mask_symbol, self.min_generate_keep,";")
        self.validset, _g, valid_copy_nums,_ = transfer(self.validset, 'multi_equation', self.mask_symbol, self.min_generate_keep,";")
        self.testset, _g, test_copy_nums,_ = transfer(self.testset, 'multi_equation', self.mask_symbol, self.min_generate_keep,";")
        ...
        if self.model.lower() in ['graph2tree']:
            if os.path.exists(self.parse_tree_path) and not self.rebuild:
                ...
                deprel_tree_to_file(self.trainset, self.validset, self.testset, self.parse_tree_path, self.language, use_gpu)
                self.trainset, self.validset, self.testset =\
                    get_group_nums_(self.trainset, self.validset, self.testset, self.parse_tree_path)
