...
from utils.enum_type import TaskType,SupervisingMode
...
def get_trainer(config):
    ...
    if sup_mode == SupervisingMode.fully_supervised:
        if config['embedding']:
            try:
                return getattr(
                    importlib.import_module('trainer.supervised_trainer'),
        ...
        try:
            return getattr(
                importlib.import_module('trainer.weakly_supervised_trainer'),
                model_name + 'WeakTrainer'
            )
        except AttributeError:
            return getattr(
        ...
        return getattr(
            importlib.import_module('trainer.abstract_trainer'),
            'AbstractTrainer'
        )
